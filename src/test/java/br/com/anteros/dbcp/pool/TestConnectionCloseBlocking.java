...
import br.com.anteros.dbcp.mocks.MockDataSource;

/**
 * Test for cases when db network connectivity goes down and close is called on existing connections. By default AnterosDBCP
...
public class TestConnectionCloseBlocking {
   ...
   public void testConnectionCloseBlocking() throws SQLException {
      ...
      try (AnterosDBCPDataSource ds = new AnterosDBCPDataSource(config);
            ...
            // AnterosDBCP only checks for validity for connections with lastAccess > 1000 ms so we sleep for 1001 ms to force
            // AnterosDBCP to do a connection validation which will fail and will trigger the connection to be closed
            quietlySleep(1100L);
