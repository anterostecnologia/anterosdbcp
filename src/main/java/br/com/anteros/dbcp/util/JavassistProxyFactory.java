...
import br.com.anteros.dbcp.pool.*;
import javassist.*;
...
public final class JavassistProxyFactory
{
   ...
   private static void modifyProxyFactory() throws NotFoundException, CannotCompileException, IOException {
      System.out.println("Generating method bodies for br.com.anteros.dbcp.proxy.ProxyFactory");

      String packageName = ProxyConnection.class.getPackage().getName();
      CtClass proxyCt = classPool.getCtClass("br.com.anteros.dbcp.pool.ProxyFactory");
      for (CtMethod method : proxyCt.getMethods()) {
         switch (method.getName()) {
            case "getProxyConnection":
               method.setBody("{return new " + packageName + ".AnterosDBCPProxyConnection($$);}");
               break;
            case "getProxyStatement":
               method.setBody("{return new " + packageName + ".AnterosDBCPProxyStatement($$);}");
               break;
            case "getProxyPreparedStatement":
               method.setBody("{return new " + packageName + ".AnterosDBCPProxyPreparedStatement($$);}");
               break;
            case "getProxyCallableStatement":
               method.setBody("{return new " + packageName + ".AnterosDBCPProxyCallableStatement($$);}");
               break;
            case "getProxyResultSet":
               method.setBody("{return new " + packageName + ".AnterosDBCPProxyResultSet($$);}");
               break;
            case "getProxyDatabaseMetaData":
               method.setBody("{return new " + packageName + ".AnterosDBCPProxyDatabaseMetaData($$);}");
               break;
            default:
               // unhandled method
               break;
   ...
   {
      String newClassName = superClassName.replaceAll("(.+)\\.(\\w+)", "$1.AnterosDBCP$2");
