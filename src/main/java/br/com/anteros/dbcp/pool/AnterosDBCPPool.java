...
import br.com.anteros.dbcp.metrics.dropwizard.CodahaleHealthChecker;
import br.com.anteros.dbcp.metrics.dropwizard.CodahaleMetricsTrackerFactory;
import br.com.anteros.dbcp.metrics.micrometer.MicrometerMetricsTrackerFactory;
import br.com.anteros.dbcp.util.ConcurrentBag;
import br.com.anteros.dbcp.util.SuspendResumeLock;
import br.com.anteros.dbcp.util.ConcurrentBag.IBagStateListener;
import br.com.anteros.dbcp.util.UtilityElf.DefaultThreadFactory;
...
import static br.com.anteros.dbcp.util.ClockSource.elapsedDisplayString;
import static br.com.anteros.dbcp.util.ClockSource.elapsedMillis;
import static br.com.anteros.dbcp.util.ClockSource.plusMillis;
import static br.com.anteros.dbcp.util.ConcurrentBag.IConcurrentBagEntry.STATE_IN_USE;
import static br.com.anteros.dbcp.util.ConcurrentBag.IConcurrentBagEntry.STATE_NOT_IN_USE;
import static br.com.anteros.dbcp.util.UtilityElf.createThreadPoolExecutor;
 ...
 * behavior for AnterosDBCPCP.
 *
 * @author Brett Wooldridge
 */
public final class AnterosDBCPPool extends PoolBase implements AnterosDBCPPoolMXBean, IBagStateListener {
	private final Logger logger = LoggerFactory.getLogger(AnterosDBCPPool.class);
	...
	private final long aliveBypassWindowMs = Long.getLong("br.com.anteros.dbcp.aliveBypassWindowMs",
			MILLISECONDS.toMillis(500));
	private final long housekeepingPeriodMs = Long.getLong("br.com.anteros.dbcp.housekeeping.periodMs",
			SECONDS.toMillis(30));
	...
	/**
	 * Construct a AnterosDBCPPool with the specified configuration.
	 *
	 * @param config a AnterosDBCPConfig instance
	 */
	public AnterosDBCPPool(final AnterosDBCPConfig config) {
		super(config);
		...
		this.addConnectionExecutor = createThreadPoolExecutor(addQueue, poolName + " connection adder", threadFactory,
		...
		this.leakTaskFactory = new ProxyLeakTaskFactory(config.getLeakDetectionThreshold(),
		...
		if (Boolean.getBoolean("br.com.anteros.dbcp.blockUntilFilled") && config.getInitializationFailTimeout() > 1) {
			...
			do {
				...
				if (poolEntry.isMarkedEvicted() || (elapsedMillis(poolEntry.lastAccessed, now) > aliveBypassWindowMs
					...
					closeConnection(poolEntry,
							poolEntry.isMarkedEvicted() ? EVICTED_CONNECTION_MESSAGE : DEAD_CONNECTION_MESSAGE);
	 ...
	 * active connections.
	 *
	 * @throws InterruptedException thrown if the thread is interrupted during
	 *                              shutdown
	 */
	...
	public void evictConnection(Connection connection) {
		...
		try {
			...
			// unreachable in AnterosDBCPCP, but we're still forced to catch it
		}
	}

	/**
	 * Set a metrics registry to be used when registering metrics collectors. The
	 * AnterosDBCPDataSource prevents this method from being called more than once.
	 *
	...
	/**
	 * Creating new poolEntry. If maxLifetime is configured, create a future
	...
	private PoolEntry createPoolEntry() {
		try {
			...
			if (poolState == POOL_NORMAL) { // we check POOL_NORMAL to avoid a flood of messages if shutdown() is
	...
	/**
	 * "Soft" evict a Connection (/PoolEntry) from the pool. If this method is being
	 * called by the user directly through
	 * {@link br.com.anteros.dbcp.AnterosDBCPDataSource#evictConnection(Connection)}
	 * then {@code owner} is {@code true}.
	 *
	...
	/**
	 * Create/initialize the Housekeeping service {@link ScheduledExecutorService}.
	 * If the user specified an Executor to be used in the
	 * {@link AnterosDBCPConfig}, then we use that. If no Executor was specified
	 * (typical), then create an Executor and configure it.
	 *
	 * @return either the user specified {@link ScheduledExecutorService}, or the
	 *         one we created
	 */
	...
	/**
	 * Destroy (/shutdown) the Housekeeping service Executor, if it was the one that
	 * we created.
	 */
	...
	private PoolStats getPoolStats() {
		return new PoolStats(SECONDS.toMillis(1)) {
			...
			protected void update() {
				this.pendingThreads = AnterosDBCPPool.this.getThreadsAwaitingConnection();
				this.idleConnections = AnterosDBCPPool.this.getIdleConnections();
				this.totalConnections = AnterosDBCPPool.this.getTotalConnections();
				this.activeConnections = AnterosDBCPPool.this.getActiveConnections();
	...
	private final class PoolEntryCreator implements Callable<Boolean> {
		...
		/**
		 * We only create connections if we need another idle connection or have threads
	...
	private final class HouseKeeper implements Runnable {
		...
		public void run() {
			try {
				...
				catalog = (config.getCatalog() != null && !config.getCatalog().equals(catalog)) ? config.getCatalog()
						: catalog;
